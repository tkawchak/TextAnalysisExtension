...
async function getSecretsAndThrowIfNotLoggedIn(functionCodes) {
  if (Object.keys(functionCodes).length === 0) {
    console.log(
      `[getDocumentText.js] Attempting to fetch function codes from auth service.`
    );
    let getSecretsResponse = await browser.runtime.sendMessage({
      action: "getsecrets",
    });
  ...
  if (Object.keys(functionCodes).length === 0) {
    console.warn(
      `[getDocumentText.js] Please login before analyzing a webpage.`
    );
...
async function popupScriptListener(message) {
  console.log(
    `[getDocumentText.js] received message from popup script: ${JSON.stringify(
      message
    )}`
  );
  ...
  if (message.command != undefined) {
    ...
    switch (command) {
      ...
      case "summarize":
        console.log("[getDocumentText.js] Summarizing current webpage.");
        await getSecretsAndThrowIfNotLoggedIn(functionCodes);
        var url = utilities.getWebpageUrl();
        var data = await client.extractWebpageInfo(url, functionCodes);
        result = await openAIClient.summarizeText(data.content, functionCodes);
        break;

      case "analyze-selected":
        console.log(
          "[getDocumentText.js] Analyzing selected text from current webpage."
        );
        await getSecretsAndThrowIfNotLoggedIn(functionCodes);
      ...
      case "explain-selected":
        console.log(
          "[getDocumentText.js] Explaining selected text from current webpage."
        );
        await getSecretsAndThrowIfNotLoggedIn(functionCodes);
      ...
      case "login":
        console.log(
          `[getDocumentText.js] Logging in user. Fetching secrets ${SECRETS}`
        );
        let loginResponse = await browser.runtime.sendMessage({
          action: "login",
          secrets: SECRETS,
        });
      ...
      case "logout":
        ...
        let logoutResponse = await browser.runtime.sendMessage({
          action: "logout",
        });
      ...
      default:
        console.log(
          `[getDocumentText.js]received unrecognized command: ${command}`
        );
...
function handleMessageFromAuthService(message) {
  if (message.loginResult != undefined) {
    console.log(
      `[getDocumentText.js] Processing auth service login result ${message.loginResult}`
    );
    functionCodes = message.secrets;
    return message.user;
  } else if (message.logoutResult != undefined) {
    console.log(
      `[getDocumentText.js] Processing auth service logout result: ${message.logoutResult}`
    );
    return "logged out";
  } else if (message.getsecretsResult != undefined) {
    console.log(
      `[getDocumentText.js] Processing auth service getsecrets result: ${message.getsecretsResult}`
    );
    functionCodes = message.secrets;
    return "fetched secrets";
  } else {
    console.warn(
      `[getDocumentText.js] Unrecognized message received from auth service: '${JSON.stringify(
        message
      )}'`
    );
